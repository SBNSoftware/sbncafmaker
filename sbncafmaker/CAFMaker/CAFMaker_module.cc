...
#include "sbncafmaker/CAFMaker/CAFMakerParams.h"
#include "sbncafmaker/CAFMaker/FillFlashMatch.h"
#include "sbncafmaker/CAFMaker/FillTrue.h"
...
// #include "sbncafmaker/CAFMaker/Blinding.h"
...
void CAFMaker::produce(art::Event& evt) noexcept {
  ...
  for (unsigned sliceID = 0; sliceID < slices.size(); sliceID++) {
    ...
    for ( size_t iPart = 0; iPart < fmPFPart.size(); ++iPart ) {
      ...
      if (!thisSplitTrack.empty()) { // it's a split track!
        rec.reco.ntrk_split ++;
      ...
      if (!thisShower.empty()) { // it's a shower!
